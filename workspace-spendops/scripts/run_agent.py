...
MODEL = "lfm2-8b-a1b"  # LM Studio uses the loaded model name; this is passed but LM Studio may ignore it
TEMPERATURE = 0.1
MAX_TOKENS = 4096
    ...
    try:
        with urllib.request.urlopen(req, timeout=120) as resp:
            ...
            if "choices" not in result:
                print(f"UNEXPECTED RESPONSE FROM LM STUDIO: {json.dumps(result, indent=2)[:500]}")
...
def validate_json(text, required_keys):
    ...
    try:
        ...
        # Accept any valid JSON from local models - log missing keys as warning only
        if required_keys:
